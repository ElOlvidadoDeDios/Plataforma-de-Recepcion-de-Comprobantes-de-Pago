...
const Sidebar = ({ isMobile, isOpen, setIsOpen }: { isMobile: boolean, isOpen: boolean, setIsOpen: (open: boolean) => void }) => {
  ...
  return (
    <div className={`bg-gradient-to-b from-cyan-500 to-blue-500 border-r border-white/20 transition-all duration-300 ease-in-out shadow-xl ${
      isMobile
        ? 'w-64 fixed top-0 left-0 z-50 h-screen flex flex-col'
        ...
        <div className="space-y-2">
          ...
          {permissions.canAccessGeodile() && (
            <NavButton
              ...
              icon={<GeodileIcon className="w-5 h-5" />}
            >
              Geodile
            </NavButton>
          )}
  ...
  return (
    <div className="min-h-screen w-full max-w-full flex flex-col bg-gradient-to-b from-cyan-500 to-blue-500 overflow-x-hidden">
      <div className="w-full flex flex-col flex-grow bg-gradient-to-r from-cyan-500 to-blue-500">
        {/* Header - Oculto cuando layoutHidden es true en Geodile */}
        ...
        {isGeodilePage && (
          <button
            onClick={() => {
              ...
              // Enviar evento personalizado para comunicar el cambio al componente Geodile
        ...
        {!permissions.isBasicUser() && !isHome && isMobile && !(isGeodilePage && layoutHidden) && (
          <button
        ...
        {!permissions.isBasicUser() && !isHome && isMobile && sidebarMobileOpen && !(isGeodilePage && layoutHidden) && (
          <div
        ...
        <div className="flex flex-row flex-grow min-h-0 relative">
          {/* Sidebar - visible solo para usuarios no básicos, cuando no está en la página principal, y cuando no está oculto en Geodile */}
          {!permissions.isBasicUser() && !isHome && !(isGeodilePage && layoutHidden) && (
            <>
              {/* Sidebar Desktop */}
              {!isMobile && (
          ...
          <div className="flex-grow overflow-hidden transition-all duration-300 ease-in-out">
            {fullWidth ? (
              // Para componentes que requieren ancho completo (como Welcome y Geodile)
      ...
      {/* Footer simple - Oculto cuando layoutHidden es true en Geodile */}
      {!(isGeodilePage && layoutHidden) && (
