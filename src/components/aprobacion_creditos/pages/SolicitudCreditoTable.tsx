...
const SolicitudCreditoTable: React.FC = () => {
  ...
  // Filtrar solicitudes cuando cambia el término de búsqueda
  useEffect(() => {
    if (!searchTerm.trim()) {
  ...
  const noResults = !isLoading && solicitudes.length > 0 && filteredSolicitudes.length === 0;
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {['Nro', 'Agencia', 'Nro_sol', 'Fecha', 'Cuenta', 'Razón Social', 'Monto Sol.', 'Moneda', 'Neto', 'Cod Cargo', 'TEM %', 'TEA %', 'Cuo Seguro', 'Acciones'].map((col) => (
                ...
                <tbody className="divide-y divide-gray-50 bg-white">
                  {filteredSolicitudes.map((sol) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-purple-600 font-medium whitespace-nowrap">{sol.AGENCIA_NOM}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{sol.COD_CARGO}</td>
                      <td className="px-4 py-3 text-sm text-orange-600 whitespace-nowrap">{parseFloat(sol.TEM).toFixed(2)}%</td>
                      <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{parseFloat(sol.TEA_INTERES).toFixed(2)}%</td>
                      <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">S/ {fmt(sol.CUO_SEGURO)}</td>
