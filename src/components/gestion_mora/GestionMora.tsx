...
const GestionMora = () => {
  ...
  const [selectedAdministrador, setSelectedAdministrador] = useState('');
  ...
  const determinarRolUsuario = () => user?.role || '';
    ...
    try {
      ...
      if (response && response.status === false) {
        ...
        const clientesOrdenados = response.data_mora.sort((a: ClienteMora, b: ClienteMora) =>
          parseInt(b.CREDITO_MORA.DIAS_ATRASO) - parseInt(a.CREDITO_MORA.DIAS_ATRASO),
        );
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className={`px-3 py-1 rounded-lg text-sm font-medium transition-all ${
              viewMode === 'cards' ? 'bg-blue-100 text-blue-700 border-2 border-blue-300' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            📋 Cards
          </button>
          <button
            ...
            className={`px-3 py-1 rounded-lg text-sm font-medium transition-all ${
              viewMode === 'table' ? 'bg-blue-100 text-blue-700 border-2 border-blue-300' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            📊 Tabla
          </button>
        ...
        {(userRole === 'SUPER_ADMIN' || userRole === 'GERENTE_GENERAL') && (
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => { setSelectedAdministrador(e.target.value); setSelectedAnalista(''); }}
              ...
              filteredClientes.map((cliente, index) => {
                ...
                return (
                  <tr key={`mora-row-${cliente.CREDITO_MORA.CUENTA}-${cliente.CREDITO_MORA.PAGARE}-${index}`} className="hover:bg-gray-50 transition-colors duration-200">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-blue-400 to-purple-500 rounded-full flex items-center justify-center">
                          <span className="text-white font-bold text-sm">{cliente.CREDITO_MORA.SOCIO.charAt(0)}</span>
                        </div>
                        <div className="ml-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="text-sm font-bold text-gray-900">{cliente.CREDITO_MORA.DIAS_ATRASO} días</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-xl shadow-lg p-12 text-center">
              ...
              <p className="text-gray-600">
                {clientes.length === 0 ? 'Seleccione un analista para ver los datos de mora.' : 'Intente ajustar los filtros de búsqueda.'}
              </p>
            </div>
